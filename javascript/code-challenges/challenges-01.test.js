...
const allUpperCase = (arr) =>
{
  ...
  arr.forEach((value, index) =>
  {
    newArray[index] = arr[index].toUpperCase();
  });
...
const speaker = (words, callback) =>
{
  ...
  {
    newArray[index] = callback(words[index]);
  });
...
const createList = (availableItems) =>
{
  ...
  {
    let currentItem = availableItems[index];

    if (currentItem.available === true)
    {
      newArray.push(currentItem.name);
    }
  });
...
describe('Testing challenge 6', () =>
{
  const inventory = [{name: 'apples', available: true}, {name: 'pears', available: true}, {
    name: 'oranges',
    available: false
  }, {name: 'bananas', available: true}, {name: 'blueberries', available: false}];
